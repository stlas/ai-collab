...
restore_session() {
    ...
    echo -e "${CYAN}📅 Gestartet: $start_time${NC}"
...
end_session() {
    ...
    local operations=$(jq -r '.statistics.operations_count' "$session_file")
    local total_cost=$(jq -r '.statistics.total_cost' "$session_file")
    local duration=$(jq -r 'if .end_time and .start_time then (((.end_time | fromdateiso8601) - (.start_time | fromdateiso8601)) / 3600) else 0 end' "$session_file")
