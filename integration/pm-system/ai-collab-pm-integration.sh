...
case "${1:-help}" in
  ...
  # Development-Cost-Export function
  export_dev_costs() {
      echo "📊 Exportiere Development-Costs..."
      if [ -f "$PROJECT_ROOT/local/development/dev_costs.json" ]; then
          cp "$PROJECT_ROOT/local/development/dev_costs.json" "$DATA_EXPORT_DIR/"
          echo "✅ Development-Costs exportiert"
      fi
  }
